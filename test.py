...
def evaluate(args):
    ...
    model = SEG_UNet(input_channels=3).to(device)
    ...
    with torch.no_grad():
        for i, (data, mask) in enumerate(tbar):
            ...
            _, pred = model(data, warm_flag=True)
            ...
            tbar.set_description(f"Eval | IoU: {current_iou * 100:.2f}%")
    ...
    with open(out_txt, 'a') as f:
        ...
        f.write(f"IoU: {final_iou * 100:.2f}% | Pd: {PD[0] * 100:.2f}% | Fa: {FA[0] * 1e6:.2f}\n\n")
